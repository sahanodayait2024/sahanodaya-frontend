...
const PastProjects = () => {
  ...
  useEffect(()=> {
    const fetcData = async () => {
      try {
        let list = [];
        const querySnapshot = await getDocs(collection(db, "pastProjects"));
  ...
  const onFinish = (values) => {
    ...
    setTimeout( async () => {
      ...
      const docRef = await addDoc(collection(db, "pastProjects"), {
       ...values
      });
  ...
  const onDeleteStudent =  async (record) => {
    Modal.confirm({
      ...
      onOK: async () => {
        ...
        await deleteDoc(doc(db, "pastProjects", record.id));
        setData(pre=>{
